...
#include "Rendering/gaussiancloud.h"
...
bool GaussianCloud::ImportPly(const std::string& plyFilename)
{
...

    {
        ...
        {
            SPDLOG_ERROR("Error parsing ply file \"%s\"\n", plyFilename.c_str());
            return false;
        }
    }
...

    {
        ...
        {
            SPDLOG_ERROR("Error parsing ply file \"%s\", missing position property\n", plyFilename.c_str());
        }
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                SPDLOG_ERROR("Error parsing ply file \"%s\", missing f_dc property\n", plyFilename.c_str());
            }
        }
        ...
        if (opt.importFullSH)
        {
            ...
            for (int i = 0; i < 45; i++)
            {
                if (!ply.GetProperty("f_rest_" + std::to_string(i), props.f_rest[i]))
                {
                    ...
                    SPDLOG_INFO("PLY file \"%s\", missing f_rest property\n", plyFilename.c_str());
                    hasFullSH = false;
                    break;
                }
            }
        ...
        {
            SPDLOG_ERROR("Error parsing ply file \"%s\", missing opacity property\n", plyFilename.c_str());
        }
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                SPDLOG_ERROR("Error parsing ply file \"%s\", missing scale property\n", plyFilename.c_str());
            }
        }
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                SPDLOG_ERROR("Error parsing ply file \"%s\", missing rot property\n", plyFilename.c_str());
            }
        }
