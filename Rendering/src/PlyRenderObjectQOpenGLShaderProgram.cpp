...
void PlyRenderObject::BuildVertexArrayObject() { 
  ...
  // gaussianDataBuffer = QOpenGLBuffer(QOpenGLBuffer::VertexBuffer);  // Mind: use 'IndexBuffer' here
 ...
 // gaussianDataBuffer.bind(); 
  ...
  SetupAttrib(GetAttribLoc("position"), gaussianCloud->GetPosWithAlphaAttrib(), 4,
              stride);
  SetupAttrib(GetAttribLoc("r_sh0"), gaussianCloud->GetR_SH0Attrib(), 4, stride);
  SetupAttrib(GetAttribLoc("g_sh0"), gaussianCloud->GetG_SH0Attrib(), 4, stride);
  SetupAttrib(GetAttribLoc("b_sh0"), gaussianCloud->GetB_SH0Attrib(), 4, stride);
  if (gaussianCloud->HasFullSH()) {
    SetupAttrib(GetAttribLoc("r_sh1"), gaussianCloud->GetR_SH1Attrib(), 4, stride);
    SetupAttrib(GetAttribLoc("r_sh2"), gaussianCloud->GetR_SH2Attrib(), 4, stride);
    SetupAttrib(GetAttribLoc("r_sh3"), gaussianCloud->GetR_SH3Attrib(), 4, stride);
    SetupAttrib(GetAttribLoc("g_sh1"), gaussianCloud->GetG_SH1Attrib(), 4, stride);
    SetupAttrib(GetAttribLoc("g_sh2"), gaussianCloud->GetG_SH2Attrib(), 4, stride);
    SetupAttrib(GetAttribLoc("g_sh3"), gaussianCloud->GetG_SH3Attrib(), 4, stride);
    SetupAttrib(GetAttribLoc("b_sh1"), gaussianCloud->GetB_SH1Attrib(), 4, stride);
    SetupAttrib(GetAttribLoc("b_sh2"), gaussianCloud->GetB_SH2Attrib(), 4, stride);
    SetupAttrib(GetAttribLoc("b_sh3"), gaussianCloud->GetB_SH3Attrib(), 4, stride);
  }
  SetupAttrib(GetAttribLoc("cov3_col0"), gaussianCloud->GetCov3_Col0Attrib(), 3,
              stride);
  SetupAttrib(GetAttribLoc("cov3_col1"), gaussianCloud->GetCov3_Col1Attrib(), 3,
              stride);
  SetupAttrib(GetAttribLoc("cov3_col2"), gaussianCloud->GetCov3_Col2Attrib(), 3,
              stride);
...
//  glDisable(GL_FRAMEBUFFER_SRGB);
 ...
 /* if (!splatRenderer->Init(gaussianCloud, isFramebufferSRGBEnabled, useRgcSortOverride)) {
  ...
  if (!splatProg->addCacheableShaderFromSourceFile(
          QOpenGLShader::Vertex,  ":/resources/shaders/splat_vertQOpenGLShaderProgram.glsl")) {
    SPDLOG_ERROR("splatProg Vertex shader error! {}", splatProg->log().toStdString());
  }
  if (!splatProg->addCacheableShaderFromSourceFile(
          QOpenGLShader::Geometry, ":/resources/shaders/splat_geomQOpenGLShaderProgram.glsl")) {
    SPDLOG_ERROR("splatProg Geom shader error! {}", splatProg->log().toStdString());
  }
  if (!splatProg->addCacheableShaderFromSourceFile(
          QOpenGLShader::Fragment,  ":/resources/shaders/splat_fragQOpenGLShaderProgram.glsl")) {
  ...
  glFuncs->glGetProgramiv(splatProg->programId(), GL_ACTIVE_ATTRIBUTES, &numAttribs);
  for (int i = 0; i < numAttribs; ++i) {
    Variable v;
    GLsizei strLen;
    glFuncs->glGetActiveAttrib(splatProg->programId(), i, MAX_NAME_SIZE, &strLen, &v.size, &v.type,
                               name);
    v.loc = glFuncs->glGetAttribLocation(splatProg->programId(), name);
     attribs[name] = v;
  }   
  ...
  if (useMultiRadixSort) {
    ...
    if (!sortProg->addShaderFromSourceFile(
            QOpenGLShader::Compute, ":/resources/shaders/multi_radixsort.glsl")) {
    ...
    if (!histogramProg->addShaderFromSourceFile(
            QOpenGLShader::Compute,
                                                    ":/resources/shaders/multi_radixsort_histograms.glsl")) {
    ...
    if (!histogramProg->bind()) {
      SPDLOG_ERROR("histogramProg Failed to bind shader program! {}", histogramProg->log().toStdString());
    } 
  } 
  // build posVec
...
void PlyRenderObject::Sort(const QMatrix4x4& cameraMat, const QMatrix4x4& projMat,
  ...
  constexpr uint32_t MAX_DEPTH = std::numeric_limits<uint32_t>::max();
  { 
...
void PlyRenderObject::draw(const std::shared_ptr<RenderData> renderData ) {
 ...
 QVector4D viewport(0.0f, 0.0f, (float)viewportSize.width(), (float)viewportSize.height());
  ...
  gsprojectionMatrix.perspective(
      renderData->fieldOfViewAngle(),
      static_cast<float>(viewportSize.width()) / static_cast<float>(viewportSize.height()), znear,
      zfar);


        Sort(renderData->viewMatrix(), gsprojectionMatrix,  nearFar);

  GL_ERROR_CHECK("SplatRenderer::Render() begin"); 
  {  
    QMatrix4x4 viewMat = renderData->viewMatrix().inverted();
    QVector3D eye = renderData->viewMatrix().column(3).toVector3D();
    splatProg->bind();
